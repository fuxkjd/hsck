...
class HostChecker:
    ...
    def _extract_host_info(self, html: str, host: str) -> bool | str:
        ...
        if len(html) > 20000 and soup.find(class_="stui-warp-content") and "最近更新" in html:
            ...
            thumb = soup.find(
                ...
                href=re.compile(r"^/(vodplay|v)/\d+-1-1\.html$"),
    ...
    def check_host(self) -> bool:
        test_host = self.config.get_saved_host() or "http://hsck.us"

        while True:
            logger.info(f"正在检查主机: {test_host}")
            html = self.client.get_html(test_host)
            result = self._extract_host_info(html, test_host)

            if result is True:
                self.config.save_host(test_host)
                logger.info(f"找到有效主机: {test_host}")
                return True
            elif isinstance(result, str):
                test_host = result
            else:
                logger.error(f"主机 {test_host} 不可用")
                return False
